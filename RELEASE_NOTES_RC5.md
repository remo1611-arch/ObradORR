# ObradORR 1.0.0-rc.5

Release candidate de ObradORR.

## Corrección principal

Corrección gastronómica-documental P0 derivada de la auditoría de los Lotes 1A + 1B.

## Cambios incluidos

- Bavaroise, Panna cotta y Tarta fría pasan a pendientes y se reclasifican como elaboraciones frías lácteas/semifríos, no como pescado cocinado.
- Gelatina “cola de pescado” mantiene pescado como alérgeno pendiente separado del APPCC.
- Ceviche y Maki mantienen advertencia de materia prima apta para consumo crudo/control anisakis.
- Pan integral sin gluten con semillas elimina referencias residuales a cerveza/gluten como ingrediente incorporado.
- Pad thai, Quiche lorraine y Mezcla de harinas sin gluten base siguen no aptas para uso docente final.
- Cordon bleu, Pechuga villeroy, Berenjenas fritas y Falafel documentan el aceite como medio de fritura/pedido, con absorción pendiente.
- Torta de nata, Croissant con poolish y Panettone modernista simplificado mantienen prefermento/rendimiento pendiente sin inventar datos.
- La impresión muestra avisos de ficha pendiente/no apta, rendimiento pendiente y prefermento pendiente.

## Estado

Esta versión es release candidate. No es versión estable final.

## Uso online

https://remo1611-arch.github.io/ObradORR/

## Reset local

https://remo1611-arch.github.io/ObradORR/app/reset_local_data.html

## SHA256 del ZIP

Se indica externamente en el mensaje de publicación.
