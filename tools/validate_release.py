...
for forbidden in ['Preflight documental experimental','Vía B ·','B1 ·','B2 ·','B3 ·','B4 ·']:
    ok(f'texto público ausente: {forbidden}') if forbidden not in blob else fail(f'texto público no limpiado: {forbidden}')
...
for k,v in [('app_version','2.0.0'),('schema_version','2.0.0'),('release_tag','2.0.0-stable'),('cache_tag','obradorr-200-stable'),('stable','1'),('public_release','1')]:
...
ok('migración pública final registrada') if cur.execute("SELECT 1 FROM migrations_log WHERE id='20260605_200_public_release_cleanup'").fetchone() else fail('migración pública final ausente')
con.close()
