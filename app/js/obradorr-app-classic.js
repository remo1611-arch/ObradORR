...
window.__OBRADORR_MODULE_VERSION = 'obradorr-210-rc9-release-candidate';
        ...
        <div class="card print-ux-main-card">
          <div class="print-ux-block">
            ...
            <div class="radio-row print-doc-row">
              ${docRadio('fichas_pedido', 'Fichas + pedido de producción', 'Documento completo de práctica: fichas y pedido de producción consolidados.')}
              ${docRadio('fichas', 'Fichas', 'Solo fichas técnicas de elaboración.')}
              ${docRadio('pedido', 'Pedido de producción', 'Solo ingredientes consolidados para aprovisionamiento, economato o mise en place.')}
            </div>
          </div>

          <div class="print-ux-block">
            <h3><span>2</span> Modelo de ficha</h3>
            <p class="muted">Selecciona el modelo documental según la finalidad de uso, sin etiquetar por nivel educativo.</p>
            <div class="radio-row profile-row print-profile-row">
              ${documentProfileRadiosHtml()}
            </div>
            ${printModelBlocksHtml(opts, 'full')}
          </div>
          ...
          <details class="print-ux-advanced">
            <summary><b>Ajustes avanzados del modelo</b><small>Sobrescriben puntualmente la matriz de bloques: APPCC, costes, subelaboraciones y proceso.</small></summary>
            <div class="options" style="margin-top:14px">
              ${opts.documentType !== 'pedido' ? sheetOptionsHtml() : '<div class="notice">El pedido de producción expande ingredientes y no imprime proceso técnico de fichas.</div>'}
...
function documentProfileRadiosHtml() {
    const catalog = window.ObradORRDocumentProfiles ? window.ObradORRDocumentProfiles.list() : [
        { id: 'ficha_trabajo', label: 'Ficha de trabajo', help: 'Salida limpia para aula-taller: ingredientes, cantidades y proceso esencial.' },
        { id: 'ficha_tecnica', label: 'Ficha técnica', help: 'Documento ordinario de práctica con proceso, alérgenos y subelaboraciones resumidas.' },
        { id: 'ficha_ampliada', label: 'Ficha técnica ampliada', help: 'APPCC, rendimiento, subfichas, observaciones técnicas y costes.' },
        { id: 'dossier_completo', label: 'Dossier completo de producción', help: 'Escandallo, costes, APPCC, validación y trazabilidad.' }
    ];
...
function printOutputSummaryHtml(opts) {
    ...
    rows.push(['Modelo de ficha', profileLabel(profile)]);
    ...
    rows.push(['Subelaboraciones', e.documentType === 'pedido' ? 'expandidas para pedido de producción' : subrecipeModeLabel(sub)]);
    rows.push(['Proceso', e.documentType === 'pedido' ? 'no aplica' : (e.includeProcess ? 'incluido' : 'oculto')]);
    const notes = [];
    if (e.documentType === 'pedido') notes.push('El pedido de producción es un documento operativo: agrupa ingredientes consolidados y mantiene alérgenos globales.');
    ...
    if (profile === 'dossier_completo') notes.push('El dossier completo se reserva para documentación docente, auditoría o gestión de producción.');
    return `<div class="print-summary"><h4>Resumen antes de imprimir</h4><dl>${rows.map(([k,v]) => `<dt>${escapeHtml(k)}</dt><dd>${escapeHtml(v)}</dd>`).join('')}</dl>${notes.length ? `<ul>${notes.map(n => `<li>${escapeHtml(n)}</li>`).join('')}</ul>` : ''}${printModelBlocksHtml(e, 'compact')}</div>`;
}

function printModelBlocksHtml(opts, density = 'full') {
    const e = effectivePrintOptions(opts || state.printOptions);
    const profile = printProfile(e);
    const summary = window.ObradORRDocumentProfiles && window.ObradORRDocumentProfiles.blockSummary
        ? window.ObradORRDocumentProfiles.blockSummary(profile, e.documentType, e)
        : [];
    if (!summary.length)
        return '';
    const visible = summary.filter(row => row.value !== false && row.value !== 'none' && row.value !== null && typeof row.value !== 'undefined');
    const hidden = summary.filter(row => row.value === false || row.value === 'none' || row.value === null || typeof row.value === 'undefined');
    const rows = (density === 'compact' ? visible : summary).map(row => {
        const cls = (row.value === false || row.value === 'none' || row.value === null || typeof row.value === 'undefined') ? 'off' : (row.value === 'optional' ? 'optional' : 'on');
        return `<span class="block-chip ${cls}"><b>${escapeHtml(row.label)}</b><small>${escapeHtml(row.valueLabel || '')}</small></span>`;
    }).join('');
    const foot = density === 'compact' && hidden.length ? `<small class="muted">${hidden.length} bloque(s) oculto(s) en este modelo.</small>` : '';
    return `<div class="print-block-matrix ${density === 'compact' ? 'compact' : ''}"><h4>Bloques imprimibles del modelo</h4><div class="print-block-chips">${rows}</div>${foot}</div>`;
}
...
function preflightPrintHtml(result, opts) {
    ...
    if (!rows.length && mode !== 'complete') {
        return `<section class="preflight-print preflight-summary"><h2>Comprobación documental previa</h2><p>${escapeHtml(summary)}. Sin avisos del nivel mostrado para este modelo.</p></section>`;
    }
    return `<section class="preflight-print ${profile === 'dossier_completo' ? 'page-break' : 'preflight-summary'}"><h2>Comprobación documental previa</h2><p>No sustituye la validación de obrador. ${escapeHtml(summary)}. ${mode === 'complete' ? 'Detalle completo incluido en este modelo.' : 'Detalle limitado al modelo seleccionado.'}</p><table><thead><tr><th>Severidad</th><th>Aviso</th><th>Detalle</th></tr></thead><tbody>${rows.map(w => `<tr><td>${escapeHtml(w.severity)}</td><td>${escapeHtml(w.title)}</td><td>${escapeHtml(w.detail || '')}</td></tr>`).join('')}</tbody></table></section>`;
}
...
function sheetOptionsHtml() {
    ...
    return `<div class="option-row"><label><input type="checkbox" id="includeCosts" ${o.includeCosts ? 'checked' : ''}/> Mostrar costes</label><small class="muted">El modelo marca el valor inicial; este check lo sobrescribe para esta exportación.</small></div>
...
// Separación RC9: datos de documento → matriz de bloques → resolvedor común de alérgenos → secciones imprimibles → render HTML.
...
function buildPrintDocumentModel(items, opts = {}) {
    ...
    const model = {
        schema: 'ObradORRPrintDocumentModel/2.1-rc9',
        items: items.slice(),
        options,
    ...
    model.sectionPlan = window.ObradORRPrintSections ? window.ObradORRPrintSections.buildPlan(model) : [];
    return model;
}
...
async function renderPrintDocumentModel(model) {
    if (window.ObradORRPrintSections && typeof window.ObradORRPrintSections.renderDocumentBody === 'function') {
        const bodyHtml = await window.ObradORRPrintSections.renderDocumentBody(model, {
            printHeader,
            preflightPrintHtml,
            printIndexHtml,
            recipeSheetHtml,
        ...
        return printDocumentShell(bodyHtml, model.filenameBase || model.title);
    }
    const opts = model.options;
    const body = [printHeader(opts)];
    body.push(preflightPrintHtml(model.preflight, opts));
    if (opts.documentType !== 'pedido' && !isCompactProfile(opts))
...
function printViewStorageKey(id) {
    return `ObradORRPrintView:${id}`;
}
function cleanupOldPrintViewPayloads() {
    try {
        const now = Date.now();
        for (let i = localStorage.length - 1; i >= 0; i--) {
            const key = localStorage.key(i);
            if (!key || !key.startsWith('ObradORRPrintView:'))
                continue;
            try {
        ...
        console.warn('[ObradORR] No se pudo limpiar la cola de impresión independiente.', error);
    }
}
function createPrintViewPayload(html, title, filenameBase, suggestedTitle) {
    cleanupOldPrintViewPayloads();
    const id = `PV-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 9)}`;
    const payload = {
        schema: 'ObradORRPrintView/2.1-rc9',
        title: title || 'Documento',
        filenameBase: filenameBase || '',
        suggestedTitle: suggestedTitle || filenameBase || title || 'ObradORR_documento',
        html,
        createdAt: new Date().toISOString(),
        expiresAt: Date.now() + 1000 * 60 * 60 * 6
    };
    localStorage.setItem(printViewStorageKey(id), JSON.stringify(payload));
    return {
        id,
        url: `./print-view.html?doc=${encodeURIComponent(id)}&v=obradorr-210-rc9-release-candidate`
    };
}
function presentPrintDocument(html, title = 'Documento', filenameBase = '') {
    ...
    let printView = null;
    try {
        printView = createPrintViewPayload(html, title, filenameBase, suggestedTitle);
    }
    catch (error) {
        console.error('[ObradORR] No se pudo preparar print-view.html.', error);
        modalRoot.innerHTML = `<div class="modal-backdrop print-backdrop"><section class="modal print-launch-modal"><header><h2>${escapeHtml(title)}</h2><div class="actions"><button class="btn" id="closePrintViewer">Cerrar</button></div></header><div class="notice warning-block"><b>No se pudo preparar la vista independiente de impresión.</b><br>Reintenta la operación y revisa la consola del navegador si el problema persiste.</div></section></div>`;
        document.getElementById('closePrintViewer')?.addEventListener('click', () => { modalRoot.innerHTML = ''; document.title = previousTitle; });
        return;
    }
    let popupOpened = false;
    try {
        const opened = window.open(printView.url, '_blank');
        popupOpened = !!opened;
        try { if (opened) opened.opener = null; } catch (_) { }
    }
    catch (error) {
        console.warn('[ObradORR] El navegador bloqueó la apertura automática de print-view.html.', error);
    }
    if (popupOpened) {
        try { document.title = previousTitle || 'ObradORR'; } catch (_) { }
        return;
    }
    modalRoot.innerHTML = `<div class="modal-backdrop print-backdrop"><section class="modal print-launch-modal"><header><h2>${escapeHtml(title)}</h2><div class="actions"><button class="btn" id="closePrintViewer">Cerrar</button></div></header><div class="notice warning-block"><b>No se pudo abrir automáticamente la vista de impresión.</b><br>El navegador puede haber bloqueado la pestaña nueva. Ábrela manualmente con el botón inferior.</div><div class="actions print-launch-actions"><a class="btn primary" id="openPrintViewButton" href="${escapeAttr(printView.url)}" target="_blank" rel="noopener">Abrir vista independiente</a></div><p class="footer-note">RC9 usa <code>print-view.html</code> como salida oficial de impresión. Solo se muestra este aviso cuando el navegador bloquea la apertura automática.</p></section></div>`;
    ...
    (_b = document.getElementById('openPrintViewButton')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
        try { document.title = suggestedTitle; } catch (_) { }
        window.setTimeout(() => { document.title = previousTitle || 'ObradORR'; closeModal(); }, 250);
    });
}
...
function sheetStatusWarningHtml(detail, kind, opts = {}) {
    if (!detail)
        return '';
    const fullValidation = shouldPrintValidation(opts);
    const warnings = [];
    ...
    else if (releaseStatus === 'pendiente' && fullValidation) {
    ...
    if (!fullValidation) {
        if (!warnings.length)
            return '';
        return `<div class="warning-block document-status-warning critical-only"><h3>Estado documental crítico</h3><ul>${[...new Set(warnings)].map(w => `<li>${w}</li>`).join('')}</ul></div>`;
    }
...
function printHeader(opts) {
    ...
    const headerProfileLabel = opts.documentType === 'pedido' ? 'Pedido de producción' : profileLabel(profile);
    addField(fields, 'Modelo de ficha', `${headerProfileLabel} · Generado: ${generated} · ObradORR`);
    if (opts.documentType === 'pedido') {
        addField(fields, 'Tipo de documento', 'Pedido de producción');
...
function documentHelpText(opts, profile) {
    if (opts.documentType === 'pedido') return 'Pedido de producción para aprovisionamiento, economato o reparto de mise en place. Agrupa ingredientes expandidos, mantiene alérgenos globales y no sustituye la revisión docente.';
    if (profile === 'dossier_completo') return 'Dossier completo de producción con escandallo, costes, APPCC, validación documental y trazabilidad técnica.';
    if (profile === 'ficha_ampliada') return 'Ficha técnica ampliada para preparación de práctica, subelaboraciones desarrolladas, APPCC, observaciones técnicas y costes estimados.';
    if (profile === 'ficha_tecnica') return 'Ficha técnica: documento ordinario de práctica con proceso, alérgenos, conservación/servicio y subelaboraciones resumidas.';
    if (profile === 'ficha_trabajo') return 'Ficha de trabajo: ingredientes, cantidades, foto si existe y proceso esencial. Sin escandallo.';
...
async function culinarySheetHtml(item, opts, pageBreak, ctx = {}) {
    ...
    const rawPhoto = await recipePhotoDataUrl('culinary', item.sourceId);
    const photo = printBlockEnabled(opts, 'photo', true) ? rawPhoto : '';
    const mode = subrecipePrintMode(opts);
    const lines = culinaryLines(item.sourceId, scale, mode === 'ingredients');
    const detail = recipeDetail('culinary', item.sourceId);
    const allergenData = resolveRecipeAllergens('culinary', item.sourceId, ctx.visited || new Set());
    const totalCost = sum(lines.map(l => l.cost || 0));
    const lineCosts = shouldShowIngredientLineCosts(opts);
    const subrecipes = mode === 'sheets' ? await culinarySubrecipeSectionsHtml(item.sourceId, scale, opts, ctx) : '';
    const directNotice = mode !== 'none' ? culinarySubrecipeSummaryHtml(item.sourceId, scale) : '';
    ...
    ${linesTable(lines, lineCosts)}
    ${lineCosts ? `<p class="cost-line"><b>Coste estimado:</b> ${money(totalCost)}</p>` : ''}
    ${escandalloBlockHtml(lines, totalCost, opts)}
    ${directNotice}
    ${printBlockEnabled(opts, 'allergens', true) ? allergenBlockHtml(allergenData, 'Alérgenos directos y derivados consolidados') : ''}
    ${opts.includeProcess && printBlockEnabled(opts, 'process', true) ? processBlock(detail, [], 'culinary') : ''}
    ${conservationServiceHtml(detail, 'culinary', opts)}
    ${appccPrintHtml(detail, 'culinary', opts)}
    ${subrecipes}
    ${traceabilityBlockHtml(item, detail, 'culinary', opts)}
  </section>`;
}
...
async function culinarySubrecipeSheetHtml(row, opts, ctx) {
    ...
    ${linesTable(lines, lineCosts)}
    ${lineCosts ? `<p class="cost-line"><b>Coste subelaboración:</b> ${money(totalCost)}</p>` : ''}
...
// Modelos documentales de impresión y opciones efectivas
...
function preflightModeForProfile(profile, documentType) {
    if (window.ObradORRDocumentProfiles && window.ObradORRDocumentProfiles.preflightMode)
        return window.ObradORRDocumentProfiles.preflightMode(profile || 'ficha_trabajo', documentType || 'fichas_pedido');
    if (profile === 'dossier_completo') return 'complete';
    if (profile === 'ficha_ampliada' || profile === 'dossier_completo') return 'critical_high';
    if (profile === 'ficha_tecnica') return 'critical';
...
function printProfile(opts = {}) {
    return (window.ObradORRDocumentProfiles && window.ObradORRDocumentProfiles.normalize ? window.ObradORRDocumentProfiles.normalize(opts.documentProfile || 'ficha_trabajo') : (opts.documentProfile || 'ficha_trabajo'));
}
function applyProfileDefaultsToState(profile) {
    const p = window.ObradORRDocumentProfiles && window.ObradORRDocumentProfiles.normalize ? window.ObradORRDocumentProfiles.normalize(profile || 'ficha_trabajo') : (profile || 'ficha_trabajo');
...
function profileOptionDefaults(profile, documentType) {
    if (window.ObradORRDocumentProfiles)
        return window.ObradORRDocumentProfiles.defaults(profile || 'ficha_trabajo', documentType || 'fichas_pedido');
    ...
    if (profile === 'ficha_ampliada')
        return { includeCosts: false, includeProcess: true, includeAppcc: true, subrecipeMode: documentType === 'pedido' ? 'ingredients' : 'sheets', preflightMode: 'critical_high' };
    if (profile === 'ficha_tecnica')
        return { includeCosts: false, includeProcess: true, includeAppcc: true, subrecipeMode: documentType === 'pedido' ? 'ingredients' : 'ingredients', preflightMode: 'critical' };
...
function effectivePrintOptions(opts = {}) {
    ...
    if (window.ObradORRDocumentProfiles && window.ObradORRDocumentProfiles.resolve) {
        const resolved = window.ObradORRDocumentProfiles.resolve(profile, documentType, out);
        out.printBlocks = resolved.blocks;
        out.printConfig = resolved.print;
        out.includeCosts = !!(resolved.blocks && resolved.blocks.costs === true);
        out.includeAppcc = !(resolved.blocks && (resolved.blocks.appcc === false || resolved.blocks.appcc === 'none'));
        out.subrecipeMode = resolved.blocks && (resolved.blocks.subrecipes === 'sheets' || resolved.blocks.subrecipes === 'ingredients' || resolved.blocks.subrecipes === 'none') ? resolved.blocks.subrecipes : out.subrecipeMode;
        out.expandSubrecipes = out.subrecipeMode !== 'none';
    }
    return out;
}
function profileLabel(profile) {
    if (window.ObradORRDocumentProfiles)
        return window.ObradORRDocumentProfiles.label(profile || 'ficha_trabajo');
    return ({ ficha_trabajo: 'Ficha de trabajo', ficha_tecnica: 'Ficha técnica', ficha_ampliada: 'Ficha técnica ampliada', dossier_completo: 'Dossier completo de producción' })[profile] || 'Ficha de trabajo';
}
function isAuditProfile(opts) {
    const p = printProfile(opts);
    return window.ObradORRDocumentProfiles ? window.ObradORRDocumentProfiles.isAudit(p) : p === 'dossier_completo';
}
function isCompactProfile(opts) {
    const p = printProfile(opts);
    return window.ObradORRDocumentProfiles ? window.ObradORRDocumentProfiles.isCompact(p) : p === 'ficha_trabajo';
}
function printBlockValue(opts, key) {
    return opts && opts.printBlocks ? opts.printBlocks[key] : undefined;
}
function printBlockEnabled(opts, key, fallback = true) {
    const v = printBlockValue(opts, key);
    if (v === undefined || v === 'optional')
        return !!fallback;
    return !(v === false || v === 'none' || v === null);
}
function subrecipePrintMode(opts) {
    const explicit = (opts && opts.subrecipeMode) || '';
    if (explicit)
        return explicit;
    const v = printBlockValue(opts, 'subrecipes');
    return v === 'sheets' || v === 'ingredients' ? v : 'none';
}
function appccPrintMode(opts) {
    const v = printBlockValue(opts, 'appcc');
    if (v === false || v === 'none' || opts && opts.includeAppcc === false)
        return 'none';
    if (v === 'brief')
        return 'brief';
    return isCompactProfile(opts) ? 'brief' : 'complete';
}
function shouldPrintValidation(opts) {
    const v = printBlockValue(opts, 'validation');
    if (v === true)
        return true;
    if (v === 'optional')
        return !!(opts && opts.includeCosts);
    return isAuditProfile(opts);
}
function shouldPrintEscandallo(opts) {
    const v = printBlockValue(opts, 'escandallo');
    if (v === true)
        return !!(opts && opts.includeCosts);
    if (v === 'optional')
        return !!(opts && opts.includeCosts);
...
function shouldShowIngredientLineCosts(opts) {
    // RC9: si el modelo imprime escandallo, el coste se concentra en ese bloque
    // para evitar duplicar la misma tabla económica en la ficha.
    return !!(opts && opts.includeCosts) && !shouldPrintEscandallo(opts);
}
function shouldPrintTraceability(opts) {
    return printBlockEnabled(opts, 'traceability', false);
}
function shouldPrintConservation(opts) {
    return printBlockEnabled(opts, 'conservation', false);
}
...
function shouldFoldSubrecipe(row, detail, opts, ctx = {}) {
    ...
    return !isAuditProfile(opts) && subrecipeCategory(row.id, row.name) === 'BASE_TECNICA';
}
...
function bakerySheetHtml(item, opts, pageBreak, ctx = {}) {
    ...
    const rawPhoto = recipePhotoDataUrl('bakery', item.sourceId);
    const photo = printBlockEnabled(opts, 'photo', true) ? rawPhoto : '';
    ...
    const allergenBundle = resolveRecipeAllergenBundle('bakery', item.sourceId, ctx.visited || new Set());
    const componentCost = sum(components.map(c => c.estimatedCost || 0));
    const totalCost = sum(allLines.map(l => l.cost || 0)) + componentCost;
    const lineCosts = shouldShowIngredientLineCosts(opts);
    const componentMode = subrecipePrintMode(opts);
    const componentSections = componentMode === 'sheets' ? bakeryComponentsSectionsHtml(components, opts, ctx) : bakeryComponentsSummaryHtml(components, opts);
    return `<section class="print-sheet ${pageBreak ? 'page-break' : ''}">
    ...
    ${blocks.map(bakeryBlockHtml(lineCosts)).join('\n')}
    ${componentSections}
    ${printBlockEnabled(opts, 'allergens', true) ? allergenBlockHtml(allergenBundle.main, 'Alérgenos directos y derivados consolidados') : ''}
    ${printBlockEnabled(opts, 'allergens', true) ? optionalComponentAllergensHtml(allergenBundle.optional) : ''}
    ${lineCosts ? `<p class="cost-line"><b>Coste estimado:</b> ${money(totalCost)}</p>` : ''}
    ${escandalloBlockHtml(allLines.concat(components.map(c => ({ name: c.component_name, quantity: c.requiredQty || 0, unit: c.requiredUnit || '', cost: c.estimatedCost || 0, technical_note: 'componente elaborado' }))), totalCost, opts)}
    ${opts.includeProcess && printBlockEnabled(opts, 'process', true) ? bakeryProcessBlocksHtml(item.sourceId) : ''}
    ${conservationServiceHtml(detail, 'bakery', opts)}
    ${appccPrintHtml(recipeDetail('bakery', item.sourceId), 'bakery', opts)}
    ${traceabilityBlockHtml(item, detail, 'bakery', opts)}
  </section>`;
}
...
function bakeryComponentsSummaryHtml(components, opts = {}) {
    if (!components || !components.length)
        return '';
    const label = subrecipePrintMode(opts) === 'ingredients' ? 'Componentes elaborados resumidos' : 'Componentes elaborados necesarios';
    return `<div class="component-summary"><h3>${escapeHtml(label)}</h3><ul>${components.map(c => `<li><b>${escapeHtml(c.component_name || '')}</b>: ${escapeHtml(c.displayQty || displayQuantity(c.requiredQty || 0, c.requiredUnit || '').text)} · ${escapeHtml(roleLabel(c.usage_role))}${Number(c.include_in_order) ? ' · incluido en pedido' : ''}</li>`).join('')}</ul><p class="footer-note">El modelo seleccionado no desarrolla subfichas de componentes. Sus ingredientes, pedido y alérgenos permanecen consolidados cuando el motor recursivo dispone de datos suficientes.</p></div>`;
}
...
function componentSectionHtml(c, opts, ctx = {}) {
    ...
    if (c.component_type === 'culinary') {
        ...
        if (!isAuditProfile(opts) && category === 'BASE_TECNICA') {
...
function visitedWithoutCurrent(sourceType, recipeId, visited = new Set()) {
    const safeVisited = new Set([...(visited || [])]);
    safeVisited.delete(`${sourceType}:${recipeId}`);
    return safeVisited;
}
function resolveRecipeAllergenBundle(sourceType, recipeId, visited = new Set()) {
    if (sourceType === 'bakery')
        return bakeryAllergenBundle(recipeId, visitedWithoutCurrent('bakery', recipeId, visited));
    return { main: culinaryAllergenData(recipeId), optional: [] };
}
function resolveRecipeAllergens(sourceType, recipeId, visited = new Set()) {
    return resolveRecipeAllergenBundle(sourceType, recipeId, visited).main;
}
...
function appccBlock(detail, sourceType, opts = {}) {
    ...
    if (rows.length) {
        const mode = opts.appccMode || appccPrintMode(opts);
        const baseTitle = sourceType === 'bakery' ? 'APPCC docente · Panadería/Pastelería' : 'APPCC docente · Cocina';
        const content = mode === 'brief' ? appccBriefRowsHtml(rows) : appccStructuredRowsHtml(rows);
        const brief = mode === 'brief' ? ' · breve' : '';
        return `<div class="appcc-block structured-appcc"><h3>${baseTitle}${brief}</h3><p class="appcc-disclaimer"><b>Modelo docente:</b> no sustituye el manual APPCC del centro ni la ficha técnica del proveedor. Los parámetros concretos deben verificarse según el procedimiento del centro.</p>${content}</div>`;
    }
...
function conservationServiceHtml(detail, sourceType, opts = {}) {
    if (!shouldPrintConservation(opts) || !detail)
        return '';
    const text = uniquePublicText([detail.service_notes, sourceType === 'bakery' ? detail.fermentation_notes : '', sourceType === 'bakery' ? detail.preferment_notes : '']);
    if (!text.trim())
        return '';
    return `<div class="conservation-block"><h3>Conservación / servicio</h3><div class="prose">${paragraphs(text)}</div></div>`;
}
function escandalloBlockHtml(lines, totalCost, opts = {}) {
    if (!shouldPrintEscandallo(opts) || !opts.includeCosts)
        return '';
    const rows = (lines || []).filter(l => Number(l.cost || 0) > 0);
    if (!rows.length)
        return `<div class="escandallo-block"><h3>Escandallo técnico docente</h3><p class="footer-note">No hay costes unitarios suficientes para desglosar el escandallo. La ficha mantiene las cantidades técnicas.</p></div>`;
    return `<div class="escandallo-block"><h3>Escandallo técnico docente</h3><p class="footer-note">Coste orientativo para aula-taller. No incluye mano de obra, energía, gastos generales, margen comercial ni IVA.</p><table class="escandallo-table"><thead><tr><th>Concepto</th><th>Cantidad</th><th>Coste</th></tr></thead><tbody>${rows.map(l => { const q = displayQuantity(l.quantity || 0, l.unit || ''); return `<tr><td>${escapeHtml(l.name || '')}</td><td class="qty">${escapeHtml(q.text)}</td><td class="money-cell">${money(l.cost || 0)}</td></tr>`; }).join('')}</tbody><tfoot><tr><th colspan="2">Coste total estimado</th><th class="money-cell">${money(totalCost || 0)}</th></tr></tfoot></table></div>`;
}
function traceabilityBlockHtml(item, detail, sourceType, opts = {}) {
    if (!shouldPrintTraceability(opts))
        return '';
    const fields = [];
    addField(fields, 'Tipo', sourceType === 'bakery' ? 'panadería/pastelería' : 'cocina');
    addField(fields, 'ID ficha', item.sourceId);
    addField(fields, 'Modelo', profileLabel(printProfile(opts)));
    addField(fields, 'Documento', docTitle(opts.documentType));
    addField(fields, 'Estado documental', detail && detail.release_status);
    if (sourceType === 'bakery')
        addField(fields, 'Estado rendimiento', detail && detail.yield_status);
    addField(fields, 'Generado', spanishLongDate(new Date()));
    return fields.length ? `<div class="traceability-block"><h3>Trazabilidad técnica</h3><dl>${fields.map(([k, v]) => `<dt>${escapeHtml(k)}</dt><dd>${escapeHtml(v)}</dd>`).join('')}</dl></div>` : '';
}
function appccPrintHtml(detail, sourceType, opts = {}) {
    const mode = appccPrintMode(opts);
    if (mode === 'none')
        return '';
    return appccBlock(detail, sourceType, Object.assign({}, opts, { appccMode: mode }));
}
...
function printDocumentShell(content, title = 'ObradORR · Documento') {
    ...
    return `<!doctype html><html lang="es"><head><meta charset="utf-8"><base href="${escapeAttr(baseHref)}"><title>${escapeHtml(safeTitle)}</title><link rel="stylesheet" href="css/print-document.css?v=obradorr-210-rc9-release-candidate"><style>body{background:#fff}</style></head><body>${content}<footer class="print-legal-footer"><strong>ObradORR</strong> · ${escapeHtml(LEGAL_NOTICE)}</footer></body></html>`;
}
...
function clearLocalUiData() {
    ...
    state.sessions = [];
    state.printOptions = {
        documentType: 'fichas_pedido', documentProfile: 'ficha_trabajo', includeTeachingData: false, includeCosts: false, subrecipeMode: 'sheets', expandSubrecipes: true, includeProcess: true, includeAppcc: true,
        teaching: { title: '', cycle: '', module: '', group: '', date: new Date().toISOString().slice(0, 10), responsible: '', notes: '' }
    };
...
function docTitle(t) { return t === 'fichas' ? 'Fichas técnicas' : t === 'pedido' ? 'Pedido de producción' : 'Fichas técnicas + pedido de producción'; }
...
// compatibility tokens: profileRadio('ficha_trabajo' profileRadio('ficha_tecnica' profileRadio('ficha_ampliada' profileRadio('dossier_completo'
